...
var simple = simpleContract.new(42, {from:web3.eth.accounts[0], data: bytecode, gas: 0x47b760, privateFor: ["ROAZBWtSacxXQrOe3FGAqJDyJjFePR5ce4TSIzmJ0Bc="]}, function(e, contract) {
	if (e) {
		...
		if (!contract.address) {
			console.log("success: Contract transaction send: TransactionHash:[" + contract.transactionHash + "] waiting to be mined...");
		} else {
			console.log("success:" + contract.address);
			console.log("storage-root:" + eth.storageRoot(contract.address))
		}
	}
});
