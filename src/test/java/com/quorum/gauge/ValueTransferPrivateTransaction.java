...
public class ValueTransferPrivateTransaction extends AbstractSpecImplementation {
    ...
    public void sendTransaction(QuorumNode from, QuorumNode to) {
        Response.Error err = transactionService.sendPrivateTransaction(new Random().nextInt(10) + 1, from, to).toBlocking().first().getError();
    ...
    public void sendSignedTransaction(QuorumNode from, QuorumNode to) {
        Response.Error err = transactionService.sendSignedPrivateTransaction(new Random().nextInt(10) + 1, from, to).toBlocking().first().getError();
