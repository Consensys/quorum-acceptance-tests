...
public class PrivateSmartContract extends AbstractSpecImplementation {
    ...
    public void updateNewValue(String contractName, int newValue, QuorumNode source, QuorumNode target) {
        Contract c = mustHaveValue(DataStoreFactory.getSpecDataStore(), contractName, Contract.class);
        TransactionReceipt receipt = contractService.updateSimpleContract(source, target, c.getContractAddress(), newValue, Arrays.asList(PrivacyFlag.StandardPrivate)).toBlocking().first();
    ...
    public void updateSimpleContractFail(String contractName, int newValue, QuorumNode source, QuorumNode target) {
        ...
        try {
            receipt = contractService.updateSimpleContract(source, target, c.getContractAddress(), newValue, Arrays.asList(PrivacyFlag.StandardPrivate)).toBlocking().first();
