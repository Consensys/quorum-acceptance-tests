...
public class PrivateRawSmartContract extends AbstractSpecImplementation {
    ...
    @Step("DeployRaw a simple smart contract with initial value <initialValue> in <source>'s default account and it's private for <target>, named this contract as <contractName>")
    public void setupContract(int initialValue, QuorumNode source, QuorumNode target, String contractName) {
        ...
        Contract contract = rawContractService.createRawSimplePrivateContract(initialValue, source, target).toBlocking().first();

        DataStoreFactory.getSpecDataStore().put(contractName, contract);
        DataStoreFactory.getScenarioDataStore().put(contractName, contract);
    }

    @Step("ExecuteRaw <contractName>'s `set()` function with new value <newValue> in <source> and it's private for <target>")
    public void updateNewValue(String contractName, int newValue, QuorumNode source, QuorumNode target) {
        Contract c = mustHaveValue(DataStoreFactory.getSpecDataStore(), contractName, Contract.class);
        TransactionReceipt receipt = rawContractService.updateRawSimplePrivateContract(source, target, c.getContractAddress(), newValue).toBlocking().first();
