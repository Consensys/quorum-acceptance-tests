...
step('send signed private transaction from node <nodeNo> to all other nodes one by one', async function(nodeNo) {
    const assert = require('assert');
    const Web3 = require('web3');
    const cfg = require("config");
    const logger = require('tracer').console({level:cfg.logLevel()});
    const signTxn = require("lib/sign_txn")(cfg, Web3, logger);
    var res = await signTxn.sendSignedTransactionFromNode(true, nodeNo);
