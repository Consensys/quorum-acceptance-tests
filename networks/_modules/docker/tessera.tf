resource "docker_container" "tessera" {
  ...
  hostname          = format("tm%d", local.full_node_indices[count.index])
  restart           = "no"
  ...
  ports {
    internal = var.tm_networking[local.full_node_indices[count.index]].port.p2p
  }
  ports {
    internal = var.tm_networking[local.full_node_indices[count.index]].port.thirdparty.internal
    external = var.tm_networking[local.full_node_indices[count.index]].port.thirdparty.external
  }
  volumes {
    container_path = "/data"
  ...
  volumes {
    ...
    host_path      = var.tessera_datadirs[local.full_node_indices[count.index]]
  }
  dynamic "volumes" {
    for_each = lookup(var.additional_tessera_container_vol, local.full_node_indices[count.index], [])
    content {
  ...
  networks_advanced {
    ...
    ipv4_address = var.tm_networking[local.full_node_indices[count.index]].ip.private
    aliases      = [format("tm%d", local.full_node_indices[count.index])]
  }
  env = local.tm_env
  healthcheck {
  ...
  entrypoint = [
    ...
    "-c",
    <<EOF
#Tessera${local.full_node_indices[count.index] + 1}

JAVA_OPTS="-Xms128M -Xmx128M"
RUN_COMMAND="/tessera/bin/tessera"

// TODO: remove following block when Jigsaw dist is default
JAR_FILE="${lookup(var.tessera_app_container_path, local.full_node_indices[count.index], "/tessera/tessera-app.jar")}"
