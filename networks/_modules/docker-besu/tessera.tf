resource "docker_container" "tessera" {
  ...
  name              = format("%s-tm%d", var.network_name, var.hybrid-network ? local.number_of_nodes + count.index : count.index)
  depends_on        = [docker_image.local, docker_image.registry]
  image             = var.tm_networking[var.hybrid-network ? local.number_of_nodes + count.index : count.index].image.name
  hostname          = format("tm%d", var.hybrid-network ? local.number_of_nodes + count.index : count.index)
  restart           = "no"
  ...
  labels {
    ...
    value = var.hybrid-network ? local.number_of_nodes + count.index : count.index
  }
  ports {
    internal = var.tm_networking[var.hybrid-network ? local.number_of_nodes + count.index : count.index].port.p2p
  }
  ports {
    internal = var.tm_networking[var.hybrid-network ? local.number_of_nodes + count.index : count.index].port.thirdparty.internal
    external = var.tm_networking[var.hybrid-network ? local.number_of_nodes + count.index : count.index].port.thirdparty.external
  }
  ports {
    internal = var.tm_networking[var.hybrid-network ? local.number_of_nodes + count.index : count.index].port.q2t.internal
    external = var.tm_networking[var.hybrid-network ? local.number_of_nodes + count.index : count.index].port.q2t.external
  }
  volumes {
    container_path = "/data"
  ...
  networks_advanced {
    ...
    ipv4_address = var.tm_networking[var.hybrid-network ? local.number_of_nodes + count.index : count.index].ip.private
    aliases      = [format("tm%d", var.hybrid-network ? local.number_of_nodes + count.index : count.index)]
  }
  env = local.tm_env
  healthcheck {
    test         = ["CMD", "nc", "-vz", "localhost", var.tm_networking[var.hybrid-network ? local.number_of_nodes + count.index : count.index].port.thirdparty.internal]
    interval     = "5s"
    retries      = 60
    timeout      = "5s"
    start_period = "10s"
  ...
  entrypoint = [
    ...
    "-c",
    <<EOF
echo "Tessera${var.hybrid-network ? (local.number_of_nodes + count.index + 1) : count.index + 1}"
